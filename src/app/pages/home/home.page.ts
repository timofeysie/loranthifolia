    ...
    getDetails() {
        ...
        this.myDataService.getDetail(itemNameAgain,this.langChoice,false).subscribe(data => {
          ...
          console.log('error from detail',error);
            ...
            if (typeof error.url !== 'undefined') {
                this.myDataService.getLocalDetail(this.langChoice,'testy');
                if (error.url === null) {
    ...
    navigateAction(item: string, i: number) {
        ...
        if ( this.list[i].cognitive_biasLabel) {
        	this.officialTitle =  this.list[i].cognitive_biasLabel;
        } else {
        	this.officialTitle =  this.list[i].wikiMedia_label;
        }
            // this.itemName = this.route.snapshot.paramMap.get('id');
            // let backupTitle = this.route.snapshot.paramMap.get('backupTitle');
            // this.officialTitle = this.route.snapshot.paramMap.get('officialTitle');
        ...
        if (typeof this.backupTitle!== 'undefined' && this.backupTitle !== null) {
            // detail/backupTitle/qCode/officialTitle
            console.log('1. using backupTitle: this.list[i][backupTitle]',this.backupTitle);
            this.itemName = this.backupTitle;
        	//this.router.navigate(['detail/'+this.backupTitle+'/'+qCode+'/'+officialTitle]);
        } else if (typeof this.list[i]['cognitive_bias'] !== 'undefined') {
            // detail/itemRoute/qCode/officialTitle
            console.log('2. using itemRoute:  this.list[i][cognitive_bias] !== undefined.');
            this.itemName = itemRoute;
            //const url = 'detail/'+itemRoute+'/'+qCode+'/'+officialTitle;
        	//this.router.navigate([url]);
        } else {
            // detail/list[i].sortName/qCode/officialTitle
            console.log('3. using: sortName',this.list[i].sortName);
            this.itemName = this.list[i].sortName;
    ...
    /**
     * This will be used among other things to find the list of available languages for a detail page.
